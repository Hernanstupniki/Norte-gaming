...
export class ShippingService {
  ...
  async listActive(province?: string) {
    const methods = await this.prisma.shippingMethod.findMany({
      where: { isActive: true },
      orderBy: { cost: 'asc' },
    });

    if (!province) return methods;

    return methods.filter(
      (m) => m.provinces.length === 0 || m.provinces.includes(province),
    );
  }
  ...
  async update(id: string, dto: UpdateShippingMethodDto) {
    const found = await this.prisma.shippingMethod.findUnique({ where: { id } });
    if (!found) throw new NotFoundException('Método de envío no encontrado');
