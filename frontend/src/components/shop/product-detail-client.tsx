  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-4 py-8 md:px-6">
      ...
      <nav aria-label="Breadcrumb" className="mb-4 flex items-center gap-1.5 text-sm text-zinc-500">
        ...
        <span>›</span>
        <Link href={`/tienda?categoria=${encodeURIComponent(product.category)}`} className="capitalize hover:text-zinc-800 transition-colors">{product.category}</Link>
        <span>›</span>
        <Link href={`/tienda?marca=${encodeURIComponent(product.brand)}`} className="hover:text-zinc-800 transition-colors">{product.brand}</Link>
        <span>›</span>
